CREATE OR REPLACE FUNCTION haikunate(token_length integer default 4, delimiter TEXT default '-', token_hex BOOLEAN default FALSE)
RETURNS TEXT AS $$
DECLARE
...
BEGIN
  ...
  FOR i IN 1..token_length LOOP
    token := token || token_chars[floor(dbms_random.value(1, array_length(token_chars)))];
  END LOOP;
